...
type YouTubePlayer = { playVideo: () => void; pauseVideo: () => void; seekTo: (seconds: number, allowSeekAhead: boolean) => void; destroy: () => void; getCurrentTime: () => number; getDuration: () => number };
...
export default function Home(){
  ...
  const routeStopCount=useRef(0); const journeyAdvancing=useRef(false); const roomOwnerId=useRef(""); const guestRoom=useRef(false); const roomSnapshot=useRef({routeFrom:from,routeTo:to,trackCursor:0,playing:false,positionSeconds:0});
  ...
  useEffect(()=>{if(!roomId||isRoomHost)return;let cancelled=false;const sync=()=>fetch(`/api/rooms?room=${encodeURIComponent(roomId)}`).then(response=>{if(!response.ok)throw new Error("Room unavailable");return response.json();}).then(data=>{if(cancelled)return;setFrom(data.routeFrom);setTo(data.routeTo);setTrackCursor(Number(data.trackCursor)||0);setBusStopIndex(Math.min(Number(data.trackCursor)||0,routeStopCount.current+1));const target=Math.max(0,Number(data.positionSeconds)||0)+(data.playing?Math.max(0,(Date.now()-Number(data.updatedAt||Date.now()))/1000):0);const player=youtubePlayer.current;if(player&&typeof player.getCurrentTime==="function"&&Math.abs(player.getCurrentTime()-target)>2.5)player.seekTo(target,true);if(data.playing&&wantsPlayback.current)player?.playVideo();if(!data.playing)player?.pauseVideo();setShareStatus(data.playing?(wantsPlayback.current?"Listening together · synced":"Tap play to join the live song"):"Co-passenger connected · paused");}).catch(()=>{if(!cancelled)setShareStatus("Shared journey is unavailable");});sync();const timer=window.setInterval(sync,2500);return()=>{cancelled=true;window.clearInterval(timer);};},[roomId,isRoomHost]);
  ...
  useEffect(()=>{
    ...
    const createPlayer=()=>{ if(cancelled||!window.YT||!youtubeMount.current)return; if(typeof youtubePlayer.current?.destroy==="function")youtubePlayer.current.destroy();
      youtubePlayer.current=new window.YT.Player(youtubeMount.current,{height:"200",width:"200",videoId:track.youtubeId,playerVars:{controls:0,playsinline:1,rel:0,fs:0,disablekb:1,origin:window.location.origin},events:{
        ...
        onStateChange:({data})=>{setPlaying(data===1);if(data===0&&!journeyAdvancing.current&&!guestRoom.current){journeyAdvancing.current=true;setBusStopIndex(value=>Math.min(value+1,routeStopCount.current+1));setProgress(0);setCurrentSeconds(0);setTrackCursor(value=>value+1);}},
  ...
  return <main className={`journey ${mood} ${weather} biome-${destination.biome}`}>
    ...
    <aside className="coming-soon" aria-label="Coming soon"><span>MORE UPDATES</span><strong>COMING SOON</strong><i/></aside>
    <aside className="co-passenger" aria-label="Invite a co-passenger"><span>TRAVEL TOGETHER</span><strong>ADD YOUR FAVOURITE<br/>CO-PASSENGER</strong><p>{shareStatus}</p><button type="button" onClick={inviteCoPassenger}>{roomId?"SHARE INVITE":"CREATE INVITE"} ↗</button></aside>
