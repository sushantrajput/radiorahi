  ...
  useEffect(()=>{
    ...
    const createPlayer=()=>{ if(cancelled||!window.YT||!youtubeMount.current)return; if(typeof youtubePlayer.current?.destroy==="function")youtubePlayer.current.destroy();
      ...
      youtubePlayer.current=new window.YT.Player(youtubeMount.current,{height:"200",width:"200",videoId:activePlaylistId?"":track.youtubeId,playerVars,events:{
        ...
        onStateChange:({data})=>{setPlaying(data===1);if(data===1){journeyAdvancing.current=false;setPlaybackError("");if(activePlaylistId){const video=youtubePlayer.current?.getVideoData();if(video?.title)setPlaylistTrack({title:video.title,artist:video.author||"YouTube"});}}if(data===0&&!journeyAdvancing.current&&!guestRoom.current){journeyAdvancing.current=true;setBusStopIndex(value=>Math.min(value+1,routeStopCount.current+1));setProgress(0);setCurrentSeconds(0);const player=youtubePlayer.current,playlistLength=activePlaylistId?player?.getPlaylist().length||0:trackList.length;setTrackCursor(value=>{const next=playlistLength?(value+1)%playlistLength:value+1;localStorage.setItem(`raahi-sequence-${activePlaylistId||mood}`,String(next));return next;});}},
