...
type YouTubePlayer = { playVideo: () => void; pauseVideo: () => void; seekTo: (seconds: number, allowSeekAhead: boolean) => void; cuePlaylist: (options: { listType: "playlist"; list: string; index: number; startSeconds: number }) => void; getPlaylistIndex: () => number; getVideoData: () => { title?: string; author?: string }; destroy: () => void; getCurrentTime: () => number; getDuration: () => number };
...
const MIDNIGHT_PLAYLIST_ID="PLCwDI18At20k";
...
export default function Home(){
  ...
  const [playlistTrack,setPlaylistTrack]=useState({title:"Midnight Ghazals",artist:"Your YouTube playlist"});
  ...
  const routeStopCount=useRef(0); const journeyAdvancing=useRef(false); const roomOwnerId=useRef(""); const guestRoom=useRef(false); const copiedUntil=useRef(0); const roomSnapshot=useRef({routeFrom:from,routeTo:to,trackCursor:0,playing:false,positionSeconds:0});
  ...
  const playerKey=mood==="midnight"?`playlist-${MIDNIGHT_PLAYLIST_ID}`:track.youtubeId; const displayTitle=mood==="midnight"?playlistTrack.title:track.title; const displayArtist=mood==="midnight"?playlistTrack.artist:track.artist;
  const stories=useMemo(()=>[
    ...
    { kicker:"ON THIS SONG",title:displayTitle,subtitle:mood==="midnight"?`${displayArtist} · Midnight ghazals`:`${track.artist} · ${track.film}`,body:mood==="midnight"?"Selected from your dedicated after-midnight ghazal playlist on YouTube.":track.fact },
  ],[destination,routeStops,track,displayTitle,displayArtist,mood]);
  ...
  useEffect(()=>{
    ...
    const createPlayer=()=>{ if(cancelled||!window.YT||!youtubeMount.current)return; if(typeof youtubePlayer.current?.destroy==="function")youtubePlayer.current.destroy();
      const playerVars:Record<string,number|string>={controls:0,playsinline:1,rel:0,fs:0,disablekb:1,origin:window.location.origin};if(mood==="midnight"){playerVars.listType="playlist";playerVars.list=MIDNIGHT_PLAYLIST_ID;playerVars.index=trackCursor;}
      youtubePlayer.current=new window.YT.Player(youtubeMount.current,{height:"200",width:"200",videoId:mood==="midnight"?"":track.youtubeId,playerVars,events:{
        ...
        onStateChange:({data})=>{setPlaying(data===1);if(data===1&&mood==="midnight"){const video=youtubePlayer.current?.getVideoData();if(video?.title)setPlaylistTrack({title:video.title,artist:video.author||"YouTube"});}if(data===0&&!journeyAdvancing.current&&!guestRoom.current){journeyAdvancing.current=true;setBusStopIndex(value=>Math.min(value+1,routeStopCount.current+1));setProgress(0);setCurrentSeconds(0);setTrackCursor(value=>value+1);}},
