  ...
  useEffect(()=>{
    ...
    const createPlayer=()=>{ if(cancelled||!window.YT||!youtubeMount.current)return; if(typeof youtubePlayer.current?.destroy==="function")youtubePlayer.current.destroy();
      ...
      youtubePlayer.current=new window.YT.Player(youtubeMount.current,{height:"200",width:"200",videoId:activePlaylistId?"":track.youtubeId,playerVars,events:{
        ...
        onStateChange:({data})=>{setPlaying(data===1);if(data===1&&activePlaylistId){const video=youtubePlayer.current?.getVideoData();if(video?.title)setPlaylistTrack({title:video.title,artist:video.author||"YouTube"});}if(data===0&&!journeyAdvancing.current&&!guestRoom.current){journeyAdvancing.current=true;setBusStopIndex(value=>Math.min(value+1,routeStopCount.current+1));setProgress(0);setCurrentSeconds(0);setTrackCursor(value=>value+1);}},
